...
dataname=r${seqname}
CUDA_VISIBLE_DEVICES=$dev python -m torch.distributed.launch --master_port $address --nproc_per_node=$ngpu optimize.py --name=$logname-0 --checkpoint_dir $checkpoint_dir --only_mean_sym --nouse_gtpose --subdivide 3 --n_mesh 21 --n_hypo 16 --num_epochs 20 --dataname $dataname  --sil_path $sil_path --ngpu $ngpu --batch_size $batchsize --opt_tex yes
